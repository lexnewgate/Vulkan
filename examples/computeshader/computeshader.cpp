 ...
 public:
  ...
  struct {
    VkDescriptorSetLayout
        descriptorSetLayout;  // Image display shader binding layout
    VkDescriptorSet
        descriptorSetPreCompute;  // Image display shader bindings before
                                  // compute shader image manipulation
    VkDescriptorSet
        descriptorSetPostCompute;     // Image display shader bindings after
                                      // compute shader image manipulation
  ...
  struct Compute {
    ...
    VkFence fence;  // Synchronization fence to avoid rewriting compute CB if
                    // still in use
    ...
    int32_t pipelineIndex =
        0;  // Current image filtering compute pipeline index
    uint32_t queueFamilyIndex;  // Family index of the graphics queue, used for
                                // barriers
  } compute;
  ...
  void loadAssets() {
    textureColorMap.loadFromFile(
        getAssetPath() + "textures/vulkan_11_rgba.ktx",
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      VkViewport viewport = vks::initializers::viewport(
          (float)viewportWidth * 0.5f, (float)viewportHeight, 0.0f, 1.0f);
      ...
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              ...
                              &graphics.descriptorSetPreCompute, 0, NULL);
      ...
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              VK_PIPELINE_BIND_POINT_GRAPHICS,
                              graphics.pipelineLayout, 0, 1,
                              &graphics.descriptorSetPostCompute, 0, NULL);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        graphics.pipeline);

      viewport.x = (float)viewportWidth / 2.0f;
      vkCmdSetViewport(drawCmdBuffers[i], 0, 1, &viewport);
      vkCmdDrawIndexed(drawCmdBuffers[i], indexCount, 1, 0, 0, 0);
  ...
  void buildComputeCommandBuffer() {
    // Flush the queue if we're rebuilding the command buffer after a pipeline
    // change to ensure it's not currently in use
  ...
  void setupVertexDescriptions() {
    ...
    vertices.bindingDescriptions = {
        vks::initializers::vertexInputBindingDescription(
            VERTEX_BUFFER_BIND_ID, sizeof(Vertex),
  ...
  void setupDescriptorSet() {
    ...
    VK_CHECK_RESULT(vkAllocateDescriptorSets(
        device, &allocInfo, &graphics.descriptorSetPreCompute));
    ...
    VK_CHECK_RESULT(vkAllocateDescriptorSets(
        device, &allocInfo, &graphics.descriptorSetPostCompute));
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getAssetPath() + "shaders/computeshader/texture.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getAssetPath() + "shaders/computeshader/texture.frag.spv",
  ...
  void getComputeQueue() {
    ...
    // Some devices have dedicated compute queues, so we first try to find a
    // queue that supports compute and not graphics
    ...
    // If there is no dedicated compute queue, just find the first queue family
    ...
    // Compute is mandatory in Vulkan, so there must be at least one queue
  ...
  void prepareCompute() {
    ...
    std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
        vks::initializers::writeDescriptorSet(compute.descriptorSet,
                                              ...
                                              0, &textureColorMap.descriptor),
        vks::initializers::writeDescriptorSet(
            compute.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 1,
            &textureComputeTarget.descriptor)};
    ...
    for (auto& shaderName : shaderNames) {
      std::string fileName =
          getAssetPath() + "shaders/computeshader/" + shaderName + ".comp.spv";
      computePipelineCreateInfo.stage =
          loadShader(fileName, VK_SHADER_STAGE_COMPUTE_BIT);
      VkPipeline pipeline;
  ...
  void updateUniformBuffers() {
    ...
    uboVS.projection = glm::perspective(
        glm::radians(60.0f),
        (float)viewportWidth * 0.5f / (float)viewportHeight, 0.1f, 256.0f);
    glm::mat4 viewMatrix =
  ...
  void draw() {
    ...
    // Use a fence to ensure that compute command buffer has finished executin
