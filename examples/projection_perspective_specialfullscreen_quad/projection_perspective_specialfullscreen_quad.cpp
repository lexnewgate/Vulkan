...
// device local memory See "prepareVertices" for details on what's staging and
// on why to use it
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  // stages used by the pipeline and the shader resources A pipeline layout can
  // be shared among multiple pipelines as long as their interfaces match
  ...
  // discussed here) Even though this adds a new dimension of planing ahead,
  ...
  // property flags we request (e.g. device local, host visibile) Upon success
  // it will return the index of the memory type that fits our requestes memory
  ...
  void draw() {
    ...
    // The submit info structure specifices a command buffer queue submission
    // batch
    VkSubmitInfo submitInfo = {};
    submitInfo.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO;
  ...
  void prepareVertices(bool useStagingBuffers) {
    ...
    std::vector<Vertex> vertexBuffer = {
        {{left_at_any_z, top_at_any_z, Zeye}, {0.0f, 1.0f, 0.0f}}, 
	{{right_at_any_z*3, top_at_any_z, Zeye}, {0.0f, 0.0f, 1.0f}},
        {{left_at_any_z, bottom_at_any_z*3, Zeye}, {1.0f, 0.0f, 0.0f}},
        //{{right_at_any_z, bottom_at_any_z, Zeye}, {0.0f, 1.0f, 0.0f}},
        };
#endif
  ...
  void preparePipelines() {
    ...
    // pipeline Note: This is actually overriden by the dynamic states (see
    // below)
    ...
    // This example does not make use fo multi sampling (for anti-aliasing), the
    ...
    // Inpute attribute bindings describe shader attribute locations and memory
    // layouts
    ...
    shaderStages[0].module =
        loadSPIRVShader(getAssetPath() + "shaders/projection_perspective_specialfullscreen_quad/triangle.vert.spv");
    ...
    shaderStages[1].module =
        loadSPIRVShader(getAssetPath() + "shaders/projection_perspective_specialfullscreen_quad/triangle.frag.spv");
    // Main entry point for the shader
  ...
  void updateUniformBuffers() {
    ...
    uboVS.projectionMatrix = glm::perspective(
        glm::radians(fovY), (float)viewportWidth / (float)viewportHeight, near,
        far);
    // TODO (xing.xu): workaround glm::perspective to invert Y.
    uboVS.projectionMatrix[1][1] *= -1.0f;
    // uboVS.viewMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, 0.0f,
    // zoom));
    uboVS.viewMatrix = glm::mat4(1.0f);
    uboVS.modelMatrix = glm::mat4(1.0f);
    uboVS.modelMatrix = glm::rotate(uboVS.modelMatrix, glm::radians(rotation.x),
                                    glm::vec3(1.0f, 0.0f, 0.0f));
    uboVS.modelMatrix = glm::rotate(uboVS.modelMatrix, glm::radians(rotation.y),
                                    glm::vec3(0.0f, 1.0f, 0.0f));
    uboVS.modelMatrix = glm::rotate(uboVS.modelMatrix, glm::radians(rotation.z),
...
// systems, but stuff like message handling diffes
