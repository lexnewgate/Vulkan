  ...
  while (1) {
    ...
    if (prepared) {
      ...
      if (camera.type != Camera::CameraType::firstperson) {
        ...
        updateView = camera.updatePad(gamePadState.axisLeft,
                                      gamePadState.axisRight, frameTimer);
        if (updateView) {
          viewChanged();
        }
      }
...
void VulkanExampleBase::submitFrame() {
  ...
  if (!((res == VK_SUCCESS) || (res == VK_SUBOPTIMAL_KHR))) {
    if (res == VK_ERROR_OUT_OF_DATE_KHR) {
      // Swap chain is no longer compatible with the surface and needs to be
      // recreated
      windowResize();
      return;
    } else {
...
bool VulkanExampleBase::initVulkan() {
  ...
  if (settings.validation) {
    ...
    // displayed For validating (debugging) an appplication the error and
      ...
      if (endptr != args[i + 1]) {
        if (index > gpuCount - 1) {
          std::cerr << "Selected device index " << index
                    << " is out of range, reverting to device 0 (use -listgpus "
                       "to show available Vulkan devices)"
                    << std::endl;
        } else {
  ...
  // phyiscal device (so that examples can check against them)
...
HWND VulkanExampleBase::setupWindow(HINSTANCE hinstance, WNDPROC wndproc) {
  ...
  if (settings.fullscreen) {
    ...
    if ((viewportWidth != (uint32_t)screenWidth) &&
        (viewportHeight != (uint32_t)screenHeight)) {
  ...
  windowRect.top = 0L;
  windowRect.right =
      settings.fullscreen ? (long)screenWidth : (long)viewportWidth;
  windowRect.bottom =
      settings.fullscreen ? (long)screenHeight : (long)viewportHeight;
  ...
  switch (cmd) {
    case APP_CMD_SAVE_STATE:
      ...
      *((struct saved_state*)vulkanExample->app->savedState) =
      vulkanExample->state; vulkanExample->app->savedStateSize = sizeof(struct
      saved_state);
      */
      break;
...
xcb_window_t VulkanExampleBase::setupWindow() {
  ...
  xcb_create_window(connection, XCB_COPY_FROM_PARENT, window, screen->root, 0,
                    0, viewportWidth, viewportHeight, 0,
...
void VulkanExampleBase::handleEvent(const xcb_generic_event_t* event) {
  switch (event->response_type & 0x7f) {
    ...
    case XCB_CONFIGURE_NOTIFY: {
      ...
      if ((prepared) && ((cfgEvent->width != viewportWidth) ||
                         (cfgEvent->height != viewportHeight))) {
