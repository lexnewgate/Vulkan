 ...
 * example:
 * https://github.com/SaschaWillems/Vulkan/tree/master/examples/particlefire
 *
 ...
 public:
  ...
  void buildCommandBuffers() {
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
      ...
      &descriptorSets.environment, 0, NULL);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
      pipelines.environment); vkCmdBindVertexBuffers(drawCmdBuffers[i],
      VERTEX_BUFFER_BIND_ID, 1, &models.environment.vertices.buffer, offsets);
      vkCmdBindIndexBuffer(drawCmdBuffers[i], models.environment.indices.buffer,
      0, VK_INDEX_TYPE_UINT32); vkCmdDrawIndexed(drawCmdBuffers[i],
      models.environment.indexCount, 1, 0, 0, 0);
      */

      // Particle system (no index buffer)
      vkCmdBindDescriptorSets(drawCmdBuffers[i],
                              VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout,
                              0, 1, &descriptorSets.particles, 0, NULL);
      vkCmdBindPipeline(drawCmdBuffers[i], VK_PIPELINE_BIND_POINT_GRAPHICS,
                        pipelines.particles);
      vkCmdBindVertexBuffers(drawCmdBuffers[i], VERTEX_BUFFER_BIND_ID, 1,
                             &particles.buffer, offsets);
  ...
  void initParticle(Particle* particle, glm::vec3 emitterPos) {
    particle->vel =
        glm::vec4(0.0f, minVel.y + rnd(maxVel.y - minVel.y), 0.0f, 0.0f);
    particle->alpha = 0.5;
    particle->size = 1.0;
    ...
    switch (particle->type) {
      case PARTICLE_TYPE_FLAME:
        ...
        if (rnd(1.0f) < 0.05f) {
          ...
          particle->pos.x *= 0.5f;
          particle->pos.z *= 0.5f;
          particle->vel = glm::vec4(rnd(1.0f) - rnd(1.0f),
  ...
  void prepareParticles() {
    ...
    for (auto& particle : particleBuffer) {
      initParticle(&particle, emitterPos);
      particle.alpha =
          1.0f;  // 1.0f - (abs(particle.pos.y) / (FLAME_RADIUS * 2.0f));
    }
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                   ...
                                   particles.size, &particles.buffer,
                                   &particles.memory, particleBuffer.data()));

    // Map the memory and store the pointer for reuse
    VK_CHECK_RESULT(vkMapMemory(device, particles.memory, 0, particles.size, 0,
                                &particles.mappedMemory));
  }
  ...
  void loadAssets() {
    ...
    // Use a different border color (than the normal texture loader) for
    // additive blending
    samplerCreateInfo.borderColor = VK_BORDER_COLOR_FLOAT_TRANSPARENT_BLACK;
    VK_CHECK_RESULT(vkCreateSampler(device, &samplerCreateInfo, nullptr,
                                    &textures.particles.sampler));

    models.environment.loadFromFile(getAssetPath() + "models/fireplace.obj",
                                    vertexLayout, 10.0f, vulkanDevice, queue);
  }
  ...
  void setupDescriptorSets() {
    ...
    VkDescriptorImageInfo texDescriptorSmoke =
        vks::initializers::descriptorImageInfo(textures.particles.sampler,
    ...
    writeDescriptorSets = {
        ...
        vks::initializers::writeDescriptorSet(
            descriptorSets.particles, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
            &uniformBuffers.fire.descriptor),
        // Binding 1: Smoke texture
        vks::initializers::writeDescriptorSet(
            descriptorSets.particles, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
            1, &texDescriptorSmoke),
    ...
    writeDescriptorSets = {
        // Binding 0: Vertex shader uniform buffer
        vks::initializers::writeDescriptorSet(
            descriptorSets.environment, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0,
  ...
  void preparePipelines() {
    ...
    // Particle rendering pipeline
    {
      ...
      shaderStages[0] = loadShader(
          getAssetPath() + "shaders/primitive_point_particle/particle.vert.spv",
          VK_SHADER_STAGE_VERTEX_BIT);
      shaderStages[1] = loadShader(
          getAssetPath() + "shaders/primitive_point_particle/particle.frag.spv",
          VK_SHADER_STAGE_FRAGMENT_BIT);

      // Vertex input state
      VkVertexInputBindingDescription vertexInputBinding =
          vks::initializers::vertexInputBindingDescription(
              VERTEX_BUFFER_BIND_ID, sizeof(Particle),
              VK_VERTEX_INPUT_RATE_VERTEX);

      std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
          vks::initializers::vertexInputAttributeDescription(
              VERTEX_BUFFER_BIND_ID, 0, VK_FORMAT_R32G32B32A32_SFLOAT,
              offsetof(Particle, pos)),  // Location 0: Position
          vks::initializers::vertexInputAttributeDescription(
              VERTEX_BUFFER_BIND_ID, 1, VK_FORMAT_R32G32B32A32_SFLOAT,
              offsetof(Particle, color)),  // Location 1: Color
          vks::initializers::vertexInputAttributeDescription(
              VERTEX_BUFFER_BIND_ID, 2, VK_FORMAT_R32_SFLOAT,
              offsetof(Particle, alpha)),  // Location 2: Alpha
          vks::initializers::vertexInputAttributeDescription(
              VERTEX_BUFFER_BIND_ID, 3, VK_FORMAT_R32_SFLOAT,
              offsetof(Particle, size)),  // Location 3: Size
          vks::initializers::vertexInputAttributeDescription(
              VERTEX_BUFFER_BIND_ID, 4, VK_FORMAT_R32_SFLOAT,
              offsetof(Particle, rotation)),  // Location 4: Rotation
          vks::initializers::vertexInputAttributeDescription(
              VERTEX_BUFFER_BIND_ID, 5, VK_FORMAT_R32_SINT,
              offsetof(Particle, type)),  // Location 5: Particle type
      };
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.fire, sizeof(uboVS)));
    ...
    VK_CHECK_RESULT(vulkanDevice->createBuffer(
        ...
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
            VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        &uniformBuffers.environment, sizeof(uboEnv)));
  ...
  void updateUniformBuffers() {
    ...
    uboVS.projection = glm::perspective(
        glm::radians(60.0f), (float)viewportWidth / (float)viewportHeight,
        0.001f, 256.0f);
    viewMatrix = glm::translate(viewMatrix, glm::vec3(0.0f, 0.0f, zoom));

    uboVS.model = glm::mat4(1.0f);
    uboVS.model =
        viewMatrix * glm::translate(uboVS.model, glm::vec3(0.0f, 0.0f, 0.0f));
    // uboVS.model = glm::rotate(uboVS.model, glm::radians(rotation.x),
    // glm::vec3(1.0f, 0.0f, 0.0f)); uboVS.model = glm::rotate(uboVS.model,
    // glm::radians(rotation.y), glm::vec3(0.0f, 1.0f, 0.0f)); uboVS.model =
    // glm::rotate(uboVS.model, glm::radians(rotation.z), glm::vec3(0.0f,
    // 0.0f, 1.0f));
