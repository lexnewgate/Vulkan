...
static float xBiggest = -100.0;
static float xSmallest = 100.0;
static float yBiggest = -100.0;
static float ySmallest = 100.0;
static float zBiggest = -100.0;
static float zSmallest = 100.0;
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  // Contains all Vulkan resources required to represent vertex and index
  // buffers for a model This is for demonstration and learning purposes, the
  // other examples use a model loader class for easy access
  struct Model {
    struct {
      VkBuffer buffer;
  ...
  // Load a model from file using the ASSIMP model loader and generate all
  // resources required to render the model
  void loadModel(std::string filename) {
    ...
    static const int assimpFlags = aiProcess_FlipWindingOrder |
    ...
    for (uint32_t m = 0; m < scene->mNumMeshes; m++) {
      for (uint32_t v = 0; v < scene->mMeshes[m]->mNumVertices; v++) {
        ...
        // Texture coordinates and colors may have multiple channels, we only
        // use the first [0] one
        ...
        if (vertex.pos.x < xSmallest)
          xSmallest = vertex.pos.x;
        if (vertex.pos.y > yBiggest)
          yBiggest = vertex.pos.y;
        if (vertex.pos.y < ySmallest)
          ySmallest = vertex.pos.y;
        if (vertex.pos.z > zBiggest)
          zBiggest = vertex.pos.z;
        if (vertex.pos.y < zSmallest)
          zSmallest = vertex.pos.z;
    ...
    if (useStaging) {
      ...
      VK_CHECK_RESULT(vulkanDevice->createBuffer(
          ...
          &model.vertices.buffer, &model.vertices.memory));
      // Index buffer
      VK_CHECK_RESULT(vulkanDevice->createBuffer(
          VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
          VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBufferSize,
          &model.indices.buffer, &model.indices.memory));
      ...
      vkCmdCopyBuffer(copyCmd, vertexStaging.buffer, model.vertices.buffer, 1,
                      &copyRegion);

      copyRegion.size = indexBufferSize;
      vkCmdCopyBuffer(copyCmd, indexStaging.buffer, model.indices.buffer, 1,
                      &copyRegion);
      ...
      VK_CHECK_RESULT(vulkanDevice->createBuffer(
          ...
          &model.vertices.buffer, &model.vertices.memory, vertexBuffer.data()));
      // Index buffer
      VK_CHECK_RESULT(vulkanDevice->createBuffer(
          VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
          indexBufferSize, &model.indices.buffer, &model.indices.memory,
          indexBuffer.data()));
    }
  }

  void loadAssets() {
    loadModel(getAssetPath() + "models/meshsphere.dae");
    if (true) {
      textures.colorMap.loadFromFile(getAssetPath() + "textures/gorilla.ktx",
                                     VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice,
                                     queue);
    } else if (deviceFeatures.textureCompressionBC) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "models/voyager/voyager_bc3_unorm.ktx",
          VK_FORMAT_BC3_UNORM_BLOCK, vulkanDevice, queue);
    } else if (deviceFeatures.textureCompressionASTC_LDR) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "models/voyager/voyager_astc_8x8_unorm.ktx",
          VK_FORMAT_ASTC_8x8_UNORM_BLOCK, vulkanDevice, queue);
    } else if (deviceFeatures.textureCompressionETC2) {
      textures.colorMap.loadFromFile(
          getAssetPath() + "models/voyager/voyager_etc2_unorm.ktx",
          VK_FORMAT_ETC2_R8G8B8A8_UNORM_BLOCK, vulkanDevice, queue);
    } else {
      vks::tools::exitFatal(
  ...
  void setupDescriptorSet() {
    ...
    VkDescriptorImageInfo texDescriptor =
        vks::initializers::descriptorImageInfo(textures.colorMap.sampler,
  ...
  void prepareUniformBuffers() {
    ...
    VK_CHECK_RESULT(
        vulkanDevice->createBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                       VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
                                   &uniformBuffers.scene, sizeof(uboVS)));
  ...
  void updateUniformBuffers() {
    ...
    // rotation.y = rotation.y + rotationSpeed;
    uboVS.model = glm::rotate(uboVS.model, glm::radians(0.0f),
